	...
	foreach($array as $index => $array_value) {
		switch($index) {
			...
			case 'mdocs-last-modified':
				...
				break;
			default:
				if(is_string($array[$index])) $array[$index] = sanitize_text_field($array_value);
				break;
		}
	}
	return $array;
...
function mdocs_process_file($file, $import=false) {
	...
	if($import == false) {
		...
		if($result == false) $result = @rename($file['tmp_name'], $upload['file']);
		if($result == false) $upload['error'] = __('An Error has occured.','memphis-documents-library');
	} else {
...
function mdocs_get_the_folder($current_folder=null, $shortcode=null) {
	...
	elseif(isset($_GET['mdocs-cat']) && $current_folder == null) $current_folder = $_GET['mdocs-cat'];
...
function mdocs_get_permalink($id, $override_admin=false) {
	if(!is_admin() || $override_admin) {
		$permalink = htmlspecialchars(get_permalink($id));
