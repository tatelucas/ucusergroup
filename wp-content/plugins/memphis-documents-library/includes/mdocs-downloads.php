...
function mdocs_load_plugins_for_download() {
	...
	if(isset($_GET['is-google'])) {
		$is_google = true;
		$the_mdoc = get_the_mdoc_by(basename($_GET['is-google']), 'id');
	...
	if(isset($_GET['mdocs-export-file']) ) {
		...
		$the_mdoc = get_the_mdoc_by(basename($_GET['mdocs-file-id']), 'id');
		$is_allowed = mdocs_check_file_rights($the_mdoc, false);
		$file = $upload_dir['basedir'].'/mdocs/'.basename($_GET['mdocs-version']);
