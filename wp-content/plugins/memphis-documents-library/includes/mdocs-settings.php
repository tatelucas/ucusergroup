...
function mdocs_script() {
	...
	if(isset($post)) {
		...
		if(get_post_type($post) == 'mdocs-posts' || has_shortcode( $post->post_content, 'mdocs' ) || is_home() || is_plugin_active('so-widgets-bundle/so-widgets-bundle.php')) {
			...
			//BOOTSTRAP
			wp_register_style( 'bootstrap.min.css', MDOC_URL.$bootstrap_css_file);
			wp_enqueue_style( 'bootstrap.min.css' );
			if(get_option('mdocs-disable-bootstrap') == false && has_shortcode( $post->post_content, 'mdocs' )  || is_plugin_active('so-widgets-bundle/so-widgets-bundle.php')) {
