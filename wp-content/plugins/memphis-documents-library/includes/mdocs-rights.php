...
function mdocs_goto_post_rights($the_mdoc, $the_mdoc_permalink) {
	if(mdocs_check_post_rights($the_mdoc)) {
		?>
		<li role="presentation"><a role="menuitem" tabindex="-1" href="<?php echo str_replace('?mdocs-cat=','', $the_mdoc_permalink); ?>" target="_blank"><i class="fa fa-arrow-circle-o-right"></i> <?php _e('Goto Post','memphis-documents-library'); ?></a></li>
		<?php
	}
}
...
function mdocs_download_rights($the_mdoc) {
	if(mdocs_check_file_rights($the_mdoc, false) && $the_mdoc['non_members'] == 'on' || is_user_logged_in()) { ?>
		...
		<li role="presentation">
			<a role="menuitem" tabindex="-1" href="<?php echo wp_login_url(htmlspecialchars(get_permalink($the_mdoc['parent']))); ?>"><i class="fa fa-cloud-download"></i> <?php _e('Login to Download','memphis-documents-library'); ?></a>
		</li>
		<?php
	}
}
